...
long long int _arr_countArr(Array * a, Array * b)
{
    ...
    for(long long int i=0;i<a->len-b->len;i++){
        if(_arr_isEqual(_arr_substr(a,i,i+b->len),b)){
            c++;
        }
    }
    return c;
...
void _arr_ins(Array ** a, long long int n, long long int i)
{
    i%=(*a)->len+1;
    _arr_pub(a,n);
    _arr_rearr(*a,(*a)->len-1,i);
}

void _arr_insArr(Array ** a, Array * b, long long int i)
{
    i%=(*a)->len+1;
    *a=_arr_concat(_arr_concat(_arr_substr((*a),0,i),b),_arr_substr((*a),i,(*a)->len));
}
